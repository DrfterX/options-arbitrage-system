...
# 自适应刷新间隔（P0.4 交易时段检测）
# ============================================================
# 根据市场交易状态自动调整调度器的数据刷新频率：
#   - refresh_day:      日盘/夜盘交易时段 — 快速刷新（默认 30 分钟）
#   - refresh_gap:      交易日间隙（15:00-21:00, 23:00-09:00）— 慢速刷新（默认 120 分钟）
#   - refresh_holiday:  非交易日（周末/节假日）— 保底心跳（默认 360 分钟）
REFRESH_INTERVALS = {
    "day": 30 * 60,       # 交易时段：30 分钟
    "night": 30 * 60,     # 夜盘时段：30 分钟（同日盘）
    "gap": 120 * 60,      # 交易日间隙：2 小时
    "holiday": 360 * 60,  # 非交易日：6 小时
}
...
ARCHIVE_RETENTION = {
    ...
    # 衍生数据（单位：天）
    "macd": 90,                # MACD 指标
    "swing_points": 90,        # 波峰波谷
    "iv_history": 180,         # 隐含波动率历史
    "signal_push_log": 30,     # 推送日志
    "filter_decision_log": 30, # 过滤决策日志
}
